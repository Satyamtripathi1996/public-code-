...
# ALB SG (only 80/443 from allowed CIDRs; egress anywhere)
resource "aws_security_group" "alb" {
  name        = "${local.name}-alb-sg"
  description = "ALB security group"
  vpc_id      = var.vpc_id

  ingress { from_port=80  to_port=80  protocol="tcp" cidr_blocks=var.allowed_ingress_cidrs }
...
# Target group (HTTP -> instance port 80)
